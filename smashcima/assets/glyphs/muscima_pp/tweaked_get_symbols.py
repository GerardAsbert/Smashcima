...
def get_full_noteheads(page: MppPage) -> List[Glyph]:
    ...
    for dataset_fold in os.listdir(actual_path):
        fold = os.path.join(os.path.join(actual_path, dataset_fold), "noteheadfull")
        if os.path.exists(fold):
            for img in os.listdir(fold):
                image = Image.open(os.path.join(fold, img)).convert('L')
                ...
                item.add("notehead", Sprite(
                    -binary_array.shape[0] // 2,
                    -binary_array.shape[1] // 2,
                    binary_array
                ))
                items.append(item)
